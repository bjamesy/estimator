...
**Decision: plain Postgres `ilike` matching via a single SQL function** (`search_line_items`, `database/migrations/0007_search_line_items.sql`), not `tsvector`/full-text search and not a dedicated search service. The function joins `line_items` → `invoices` → `projects`/`suppliers`, left-joins `material_matches` (only `status = 'proposed'` as of `0022_estimate_line_material_id.sql` — a `flagged` match's line item still appears in results, just without a material name/id) → `material_catalog`, and matches the query against material name, description, SKU, supplier name, and project name in one query, company-wide (not project-scoped) per the product spec's own search example. `0022` also added `material_id` to its return shape, alongside the `material_name` it already returned, so the estimate-builder's "Add from history" flow can link the resulting `EstimateLine` back to its canonical `MaterialCatalog` entry (see `docs/data_model.md` → EstimateLine).
