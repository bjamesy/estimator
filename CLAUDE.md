...
**v2 in-focus features complete** (branch `v2/change-orders`, migrations `0016`–`0021`; see `docs/v2/plans/00-roadmap.md` for status): change orders with immutable estimate versioning, Ontario CPA 10% flagging, dual signatures via a tokenized public `/sign/[token]` page, worker-rendered legal PDFs (placeholder template pending lawyer review), email notifications (console transport until `RESEND_API_KEY` set); contractor credential verification (V1 document-on-file with vision extraction and expiry reminders); vendor price spot-checks (allowlisted hosts, audit stamp into PDFs). Marketplace and project templates are backlogged as the final two priorities.
